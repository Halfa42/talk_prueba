...
const getTareasByBeneficiario = async (req, res) => {
  try {
    ...
    const result = await query(
      `SELECT t.*, e.calificacion, e.retroalimentacion, e.fecha_entrega, e.archivo_entregado
       FROM tarea t
       INNER JOIN asignacion a ON t.id_asignacion = a.id_asignacion
       INNER JOIN beneficiario b ON a.id_beneficiario = b.id_beneficiario
...
const downloadArchivoEntregado = async (req, res) => {
  try {
    const entregaId = Number(req.params.entregaId);
    
    const result = await query(
      'SELECT archivo_entregado FROM entrega WHERE id_entrega = $1', 
      [entregaId]
    );

    if (result.rows.length === 0 || !result.rows[0].archivo_entregado) {
      return res.status(404).json({ message: 'Archivo de entrega no encontrado' });
    }

    const archivoRuta = result.rows[0].archivo_entregado;
    const normalizedPath = archivoRuta.startsWith('/') ? archivoRuta.slice(1) : archivoRuta;
    const filePath = path.join(__dirname, '..', normalizedPath);
    ...
    console.error(error);
    return res.status(500).json({ message: 'Error interno al descargar la entrega' });
  }
};
...
const calificarEntrega = async (req, res) => {
  try {
    const entregaId = Number(req.params.entregaId);
    const { calificacion, retroalimentacion, id_tarea } = req.body;
    ...
    const result = await query(
      `UPDATE entrega 
       SET calificacion = $1, retroalimentacion = $2 
       WHERE id_entrega = $3 RETURNING *`,
    ...
    if (id_tarea) {
      await query(`UPDATE tarea SET estatus = 'calificada' WHERE id_tarea = $1`, [id_tarea]);
    }

    return res.json({ message: 'Entrega calificada con éxito', entrega: result.rows[0] });
  } catch (error) {
    console.error(error);
