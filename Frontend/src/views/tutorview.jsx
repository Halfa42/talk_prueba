...
import { LayoutDashboard, Users, BookOpen, ClipboardList, FileText, Clock3, FolderOpen, CheckCircle2, Upload } from 'lucide-react';
...
export default function TutorView() {
  ...
  const [selectedStudent, setSelectedStudent] = useState({
    name: "María López", level: "Nivel A2", program: "Inglés", status: "Activo", attendance: "92%", diagnostic: "A1", current: "A2"
  });
  ...
  const tabClass = (active) => `px-4 py-2 rounded-xl text-sm border transition ${active ? "bg-blue-600 text-white border-blue-600" : "bg-white border-slate-200 text-slate-700 hover:bg-slate-50"}`;

  const renderContent = () => {
    if (tutorModule === "dashboard") return (
          ...
          <div className="xl:col-span-2 space-y-6">
            <div className={softCard + " p-5"}>
              ...
              <div className="grid md:grid-cols-4 gap-4 text-sm">
                <button onClick={() => setTutorModule("students")} className="p-4 rounded-2xl border bg-slate-50 text-left hover:border-blue-300 transition"><Users className="mb-3" size={18} /><div className="font-medium">Mis alumnos</div><div className="text-slate-500 mt-1">Consulta perfiles y avance.</div></button>
                <button onClick={() => setTutorModule("materials")} className="p-4 rounded-2xl border bg-slate-50 text-left hover:border-blue-300 transition"><FolderOpen className="mb-3" size={18} /><div className="font-medium">Materiales</div><div className="text-slate-500 mt-1">Sube recursos por tema.</div></button>
                <button onClick={() => setTutorModule("tasks")} className="p-4 rounded-2xl border bg-slate-50 text-left hover:border-blue-300 transition"><ClipboardList className="mb-3" size={18} /><div className="font-medium">Tareas</div><div className="text-slate-500 mt-1">Asigna y revisa entregas.</div></button>
                <button onClick={() => setTutorModule("hours")} className="p-4 rounded-2xl border bg-slate-50 text-left hover:border-blue-300 transition"><Clock3 className="mb-3" size={18} /><div className="font-medium">Horas</div><div className="text-slate-500 mt-1">Consulta horas registradas.</div></button>
              </div>
            </div>
            <div className={softCard + " p-5"}>
    ...
    if (tutorModule === "students") return (
      <div className="space-y-6">
        <div className="flex items-center justify-between"><div><h2 className="text-2xl font-bold">Mis alumnos</h2><p className="text-sm text-slate-500">Consulta información general, progreso y accesos rápidos.</p></div><button className="px-4 py-2 rounded-xl bg-blue-600 text-white text-sm">Exportar lista</button></div>
    ...
    if (tutorModule === "materials") return (
      <div className="space-y-6">
        <div>
          <h2 className="text-2xl font-bold">Material para beneficiario</h2>
          <p className="text-sm text-slate-500">Carga recursos por tema, nivel y alumno.</p>
        </div>
        <div className="grid xl:grid-cols-3 gap-6">
          <div className="xl:col-span-2 space-y-6">
            <div className={softCard + " p-5"}>
              ...
              <div className="grid md:grid-cols-2 gap-4">
                <input className="rounded-xl border border-slate-300 px-4 py-3" placeholder="Título del material" />
                <input className="rounded-xl border border-slate-300 px-4 py-3" placeholder="Tema" />
                <input className="rounded-xl border border-slate-300 px-4 py-3" placeholder="Nivel" />
                <select className="rounded-xl border border-slate-300 px-4 py-3 bg-white">
                  <option>Asignar a beneficiario</option>
                </select>
              </div>
              <textarea className="mt-4 w-full rounded-xl border border-slate-300 px-4 py-3" rows="4" placeholder="Descripción del recurso" />
              <div className="mt-4 flex gap-3">
                <button className="px-4 py-2 rounded-xl bg-blue-600 text-white">Subir archivo</button>
                <button className="px-4 py-2 rounded-xl bg-slate-100">Guardar material</button>
              </div>
            </div>
          </div>
          <div className={softCard + " p-5"}>
            ...
            <div className="space-y-3 text-sm">
              <div className="p-3 rounded-xl bg-slate-50 border">Tema 1 — Presentaciones</div>
              <div className="p-3 rounded-xl bg-slate-50 border">Tema 2 — Rutinas diarias</div>
              <div className="p-3 rounded-xl bg-slate-50 border">Listening practice A2</div>
            </div>
          </div>
        </div>
      </div>
      ...
      <div className="space-y-6">
        ...
        <div className="grid xl:grid-cols-2 gap-6">
          <div className={softCard + " p-5"}>
            ...
            <input className="w-full rounded-xl border border-slate-300 px-4 py-3 mb-3" placeholder="Título de la tarea" />
            <textarea className="w-full rounded-xl border border-slate-300 px-4 py-3 mb-3" rows="4" placeholder="Instrucciones" />
            <div className="grid md:grid-cols-2 gap-3">
              <input className="rounded-xl border border-slate-300 px-4 py-3" placeholder="Beneficiario" />
              <input className="rounded-xl border border-slate-300 px-4 py-3" placeholder="Fecha límite" />
            </div>
            <button className="mt-4 px-4 py-2 rounded-xl bg-blue-600 text-white">Publicar tarea</button>
          </div>
          <div className={softCard + " p-5"}>
            <h3 className="font-semibold text-lg mb-4">Entregas por revisar</h3>
            <div className="space-y-3">
              {["Vocabulary worksheet — María López", "Reading practice — Carlos Vega", "Listening exercise — Sofía Cruz"].map((item) => (
                <div key={item} className="p-3 rounded-xl bg-slate-50 border flex items-center justify-between text-sm">
                  <span>{item}</span>
                  <button className="px-3 py-2 rounded-xl bg-white border">Calificar</button>
                </div>
              ))}
            </div>
          </div>
    ...
    if (tutorModule === "session") {
      ...
      return (
        <div className="space-y-6">
          <div className="flex items-center justify-between"><div><h2 className="text-2xl font-bold">Seguimiento de sesión</h2><p className="text-sm text-slate-500">Workspace central del tutor.</p></div><div className="flex gap-2"><button onClick={() => setTutorModule("students")} className="px-4 py-2 rounded-xl bg-slate-100 text-sm">Volver a alumnos</button><button className="px-4 py-2 rounded-xl bg-blue-600 text-white text-sm">Guardar progreso</button></div></div>
          <div className="grid xl:grid-cols-[300px,1fr,320px] gap-6">
            <div className="space-y-4">
              <div className={softCard + " p-5"}><div className="flex items-center gap-3"><div className="w-12 h-12 rounded-2xl bg-blue-100 text-blue-700 flex items-center justify-center font-semibold">{selectedStudent.name[0]}</div><div><div className="font-semibold">{selectedStudent.name}</div><div className="text-sm text-slate-500">{selectedStudent.level}</div></div></div><div className="mt-4 space-y-3 text-sm text-slate-600"><div className="flex justify-between"><span>Estado</span><span className="font-medium text-blue-700">{selectedStudent.status}</span></div><div className="flex justify-between"><span>Asistencia</span><span>{selectedStudent.attendance}</span></div></div></div>
            ...
            <div className="space-y-4">
              <div className={softCard + " p-5"}><h3 className="font-semibold mb-4">Horas</h3><div className="h-28 rounded-2xl bg-slate-50 border flex items-center justify-center text-slate-500">1.5 h en esta sesión</div></div>
              <div className={softCard + " p-5"}><h3 className="font-semibold mb-4">Bitácoras recientes</h3><div className="space-y-3 text-sm"><div className="p-3 rounded-xl bg-slate-50 border">Seguimiento: mejorar fluidez oral.</div><div className="p-3 rounded-xl bg-slate-50 border">Acuerdo: repasar verbos.</div></div></div>
            </div>
          </div>
        </div>
      );
    ...
    if (tutorModule === "hours") return (
      <div className="space-y-6">
        <div>
          <h2 className="text-2xl font-bold">Horas acumuladas</h2>
          <p className="text-sm text-slate-500">Consulta horas registradas, validadas y pendientes.</p>
        </div>
        <div className={softCard + " p-5"}>
          <div className="space-y-3 text-sm">
        ...
        <div className={softCard + " p-5"}>
          <div className="h-72 rounded-2xl bg-slate-50 border flex items-center justify-center text-slate-500">Gráfica o historial de horas por sesión</div>
        </div>
      </div>
    );
  };
  ...
  return (
    <div className="flex min-h-[860px] bg-slate-50">
      <aside className="w-72 bg-white border-r border-slate-200 p-4 space-y-2">
        <button onClick={() => setTutorModule("dashboard")} className={menuClass(tutorModule === "dashboard")}><LayoutDashboard size={18} />Dashboard</button>
        <button onClick={() => setTutorModule("students")} className={menuClass(tutorModule === "students")}><Users size={18} />Mis alumnos</button>
        <button onClick={() => setTutorModule("materials")} className={menuClass(tutorModule === "materials")}><BookOpen size={18} />Materiales</button>
        <button onClick={() => setTutorModule("tasks")} className={menuClass(tutorModule === "tasks")}><ClipboardList size={18} />Tareas y evaluación</button>
        <button onClick={() => setTutorModule("session")} className={menuClass(tutorModule === "session")}><FileText size={18} />Seguimiento de sesión</button>
        <button onClick={() => setTutorModule("hours")} className={menuClass(tutorModule === "hours")}><Clock3 size={18} />Horas acumuladas</button>
      </aside>
