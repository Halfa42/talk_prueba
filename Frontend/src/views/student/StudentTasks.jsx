    ...
    <div className="space-y-6 relative">
      ...
      <div className="grid xl:grid-cols-2 gap-6">
        <div className={softCard + " p-5 overflow-hidden flex flex-col"}>
          <h3 className="font-semibold text-lg mb-4">Mis Tareas</h3>
          <div className="overflow-x-auto">
            <table className="w-full text-left border-collapse">
              ...
              <tbody className="text-sm">
                {tareas.length === 0 ? (
                  <tr>
                    <td colSpan="3" className="text-center py-4 text-slate-400">No hay tareas asignadas</td>
                  </tr>
                ) : (
                  tareas.map(t => (
                    <tr key={t.id_tarea} className="border-b last:border-0 hover:bg-slate-50 transition">
                      <td className="py-3 font-medium flex items-center gap-2">
                        <div className={`w-2 h-2 rounded-full ${t.estatus === 'calificada' ? 'bg-purple-500' : t.estatus === 'entregada' ? 'bg-green-500' : 'bg-amber-500'}`}></div>
                        {t.titulo}
                      </td>
        ...
        <div className={softCard + " p-5"}>
          ...
          >
            ...
            {tareas.filter(t => t.estatus !== 'entregada' && t.estatus !== 'calificada').map(t => (
            ...
            <div className="p-6 space-y-4">
              ...
              <div className="grid grid-cols-2 gap-4">
                ...
                <div>
                  ...
                  <span className={`inline-block px-3 py-1 text-xs font-semibold rounded-full ${selectedTarea.estatus === 'calificada' ? 'bg-purple-100 text-purple-700' : selectedTarea.estatus === 'entregada' ? 'bg-green-100 text-green-700' : 'bg-amber-100 text-amber-700'}`}>
                    {selectedTarea.estatus === 'calificada' ? 'Calificada' : selectedTarea.estatus === 'entregada' ? 'Entregada' : 'Pendiente'}
                  </span>
                </div>
              </div>

              {selectedTarea.estatus === 'calificada' && (
                <div className="mt-4 pt-4 border-t border-slate-100">
                  <h4 className="text-sm font-semibold text-slate-500 uppercase tracking-wider mb-2">Evaluación del Tutor</h4>
                  <div className="bg-purple-50 p-4 rounded-xl border border-purple-100">
                    <div className="flex items-center justify-between mb-2">
                      <span className="text-slate-700 font-medium">Calificación:</span>
                      <span className="text-lg font-bold text-purple-700">{selectedTarea.calificacion} / 100</span>
                    </div>
                    {selectedTarea.retroalimentacion && (
                      <div className="mt-2">
                        <span className="text-sm font-medium text-slate-700 block mb-1">Comentarios:</span>
                        <p className="text-sm text-slate-600 bg-white p-3 rounded-lg border border-purple-50">
                          {selectedTarea.retroalimentacion}
                        </p>
                      </div>
                    )}
