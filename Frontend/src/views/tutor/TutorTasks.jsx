...
export default function TutorTasks({ softCard, tutorUserId }) {
  ...
  const loadAll = () => {
    fetch(`http://localhost:3000/api/tareas/beneficiarios/${tutorUserId}`)
      .then((r) => r.json())
      .then(setBeneficiarios);

    fetch(`http://localhost:3000/api/tareas/bytutor/${tutorUserId}`)
      .then((r) => r.json())
      .then(setTareas);

    fetch(`http://localhost:3000/api/tareas/entregas/${tutorUserId}`)
      .then((r) => r.json())
      .then(setEntregas);
  };
    ...
    <div className="space-y-6">
      ...
      <div className="grid xl:grid-cols-2 gap-6">
        <div className={softCard + " p-5"}>
          ...
          <input
            type="file"
            className="mt-3 w-full text-sm text-slate-600 cursor-pointer"
        ...
        <div className={softCard + " p-5"}>
          ...
          <div className="space-y-3">
            {entregas.length === 0 && (
              <div className="text-slate-400">Sin entregas pendientes</div>
            )}
            ...
            {entregas.map((e) => (
              ...
              >
                <span>
                  {e.titulo} - {e.nombre} {e.apellido_paterno}
                </span>
      ...
      <div className={softCard + " p-5"}>
        ...
        <div className="space-y-2">
          {tareas.length === 0 && (
            <div className="text-slate-400">Sin tareas asignadas</div>
          )}
          ...
          {tareas.map((t) => (
            <div key={t.id_tarea} className="p-3 rounded-xl bg-slate-50 border">
              ...
              <div className="text-xs text-slate-500">
                Para: {t.nombre} {t.apellido_paterno}
              </div>
              <div className="text-xs text-slate-500">
