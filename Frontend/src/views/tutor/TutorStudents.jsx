...
export default function TutorStudents({ softCard, onOpenStudent, tutorUserId }) {
    ...
    const fetchStudents = async () => {
      try {
        const response = await fetch(
          `http://localhost:3000/api/tutor-students?userId=${tutorUserId}`
        );
  ...
  return (
    <div className="grid md:grid-cols-2 xl:grid-cols-3 gap-4">
      {students.map((student) => (
        <div key={student.id_beneficiario} className={softCard + " p-5"}>
          <div className="flex items-start justify-between">
            <div>
              ...
              <div className="text-sm text-slate-500">
                Idioma del curso: {student.idioma_curso || "N/A"}
              </div>
